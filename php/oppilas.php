...
// Kaikkien tekemää koodia jossa käsitellään axiosista tulevia pyyntöjä
...
switch ($method) {
    case 'GET': // Harjoituksen tietojen palauttaminen OppilasID:n perusteella
...
if ($method == 'GET') {
  ...
  break;
  case 'PUT': // Harjoituksen hyväksyminen
    $url= $_SERVER['REQUEST_URI'];
      $data = basename($url);
    $path = explode('/', $_SERVER['REQUEST_URI']);
    ...
    case 'DELETE': // Harjoituksen poistaminen
      $path = explode('/', $_SERVER['REQUEST_URI']);
      $id = intval(end($path)); 
